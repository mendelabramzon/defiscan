...
def check_trans(block_start, block_finish):
    str1 = 'https://api.polygonscan.com/api?module=account&action=txlist&address=&startblock='
    str2 = '&endblock='
                ...
                for i, rout in enumerate(decoded_input[1]['stocks']):
                    ...
                    for trans in txs['transactions']:
                        try:
                            ...
                            for i in range(L):
                                pool_add = pair.events.Swap().processReceipt(receipt, errors='DISCARD')[i]['args'][
                                    'address']
                                ...
                                if fac == fac_our and ((tok_0 == toks_our[0] and tok_1 == toks_our[1]) or
                                    ...
                                    with open(file_name, 'a') as x:
                                        ...
                                        print("pool {}\nour trans {}\nhis trans {}".format(pool_add, hsh, receipt[
                                            'transactionHash'].hex()))
                                        x.write("pool {}\nour trans {}\nhis trans {}\n".format(pool_add, hsh, receipt[
                                            'transactionHash'].hex()))
                                        print("gas: our {}, their {}\n".format(our_gas, gas))
...
block_file = "current_block.txt"
t_wait = 5
while True:
    start = datetime.datetime.now()
    with open(block_file, "r") as x:
        current_block = int(x.read())
    last_block = matic.eth.get_block("latest")['number']
    check_trans(current_block, last_block)
    with open(block_file, "w") as x:
        x.write(str(last_block))
